.class final Lbpr$a;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Lbmi$g;


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lbpr;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x18
    name = "a"
.end annotation


# instance fields
.field private final a:Lcba;

.field private final b:Lcar;


# direct methods
.method private constructor <init>(Lcba;)V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p1, p0, Lbpr$a;->a:Lcba;

    new-instance p1, Lcar;

    invoke-direct {p1}, Lcar;-><init>()V

    iput-object p1, p0, Lbpr$a;->b:Lcar;

    return-void
.end method

.method synthetic constructor <init>(Lcba;Lbps;)V
    .locals 0

    invoke-direct {p0, p1}, Lbpr$a;-><init>(Lcba;)V

    return-void
.end method

.method private a(Lcar;JJ)Lbmi$f;
    .locals 9

    const/4 v0, -0x1

    const-wide v1, -0x7fffffffffffffffL    # -4.9E-324

    move-wide v3, v1

    const/4 v5, -0x1

    :goto_0
    invoke-virtual {p1}, Lcar;->b()I

    move-result v6

    const/4 v7, 0x4

    if-lt v6, v7, :cond_5

    iget-object v6, p1, Lcar;->a:[B

    invoke-virtual {p1}, Lcar;->d()I

    move-result v8

    invoke-static {v6, v8}, Lbpr;->a([BI)I

    move-result v6

    const/16 v8, 0x1ba

    if-eq v6, v8, :cond_0

    const/4 v6, 0x1

    invoke-virtual {p1, v6}, Lcar;->d(I)V

    goto :goto_0

    :cond_0
    invoke-virtual {p1, v7}, Lcar;->d(I)V

    invoke-static {p1}, Lbpt;->a(Lcar;)J

    move-result-wide v6

    cmp-long v0, v6, v1

    if-eqz v0, :cond_4

    iget-object v0, p0, Lbpr$a;->a:Lcba;

    invoke-virtual {v0, v6, v7}, Lcba;->d(J)J

    move-result-wide v6

    cmp-long v0, v6, p2

    if-lez v0, :cond_2

    cmp-long p1, v3, v1

    if-nez p1, :cond_1

    invoke-static {v6, v7, p4, p5}, Lbmi$f;->a(JJ)Lbmi$f;

    move-result-object p1

    return-object p1

    :cond_1
    int-to-long p1, v5

    add-long/2addr p4, p1

    invoke-static {p4, p5}, Lbmi$f;->a(J)Lbmi$f;

    move-result-object p1

    return-object p1

    :cond_2
    const-wide/32 v3, 0x186a0

    add-long/2addr v3, v6

    cmp-long v0, v3, p2

    if-lez v0, :cond_3

    invoke-virtual {p1}, Lcar;->d()I

    move-result p1

    int-to-long p1, p1

    add-long/2addr p4, p1

    invoke-static {p4, p5}, Lbmi$f;->a(J)Lbmi$f;

    move-result-object p1

    return-object p1

    :cond_3
    invoke-virtual {p1}, Lcar;->d()I

    move-result v0

    move v5, v0

    move-wide v3, v6

    :cond_4
    invoke-static {p1}, Lbpr$a;->a(Lcar;)V

    invoke-virtual {p1}, Lcar;->d()I

    move-result v0

    goto :goto_0

    :cond_5
    cmp-long p1, v3, v1

    if-eqz p1, :cond_6

    int-to-long p1, v0

    add-long/2addr p4, p1

    invoke-static {v3, v4, p4, p5}, Lbmi$f;->b(JJ)Lbmi$f;

    move-result-object p1

    return-object p1

    :cond_6
    sget-object p1, Lbmi$f;->a:Lbmi$f;

    return-object p1
.end method

.method private static a(Lcar;)V
    .locals 5

    invoke-virtual {p0}, Lcar;->c()I

    move-result v0

    invoke-virtual {p0}, Lcar;->b()I

    move-result v1

    const/16 v2, 0xa

    if-ge v1, v2, :cond_0

    invoke-virtual {p0, v0}, Lcar;->c(I)V

    return-void

    :cond_0
    const/16 v1, 0x9

    invoke-virtual {p0, v1}, Lcar;->d(I)V

    invoke-virtual {p0}, Lcar;->h()I

    move-result v1

    and-int/lit8 v1, v1, 0x7

    invoke-virtual {p0}, Lcar;->b()I

    move-result v2

    if-ge v2, v1, :cond_1

    invoke-virtual {p0, v0}, Lcar;->c(I)V

    return-void

    :cond_1
    invoke-virtual {p0, v1}, Lcar;->d(I)V

    invoke-virtual {p0}, Lcar;->b()I

    move-result v1

    const/4 v2, 0x4

    if-ge v1, v2, :cond_2

    invoke-virtual {p0, v0}, Lcar;->c(I)V

    return-void

    :cond_2
    iget-object v1, p0, Lcar;->a:[B

    invoke-virtual {p0}, Lcar;->d()I

    move-result v3

    invoke-static {v1, v3}, Lbpr;->a([BI)I

    move-result v1

    const/16 v3, 0x1bb

    if-ne v1, v3, :cond_4

    invoke-virtual {p0, v2}, Lcar;->d(I)V

    invoke-virtual {p0}, Lcar;->i()I

    move-result v1

    invoke-virtual {p0}, Lcar;->b()I

    move-result v3

    if-ge v3, v1, :cond_3

    invoke-virtual {p0, v0}, Lcar;->c(I)V

    return-void

    :cond_3
    invoke-virtual {p0, v1}, Lcar;->d(I)V

    :cond_4
    :goto_0
    invoke-virtual {p0}, Lcar;->b()I

    move-result v1

    if-lt v1, v2, :cond_8

    iget-object v1, p0, Lcar;->a:[B

    invoke-virtual {p0}, Lcar;->d()I

    move-result v3

    invoke-static {v1, v3}, Lbpr;->a([BI)I

    move-result v1

    const/16 v3, 0x1ba

    if-eq v1, v3, :cond_8

    const/16 v3, 0x1b9

    if-ne v1, v3, :cond_5

    goto :goto_1

    :cond_5
    ushr-int/lit8 v1, v1, 0x8

    const/4 v3, 0x1

    if-eq v1, v3, :cond_6

    goto :goto_1

    :cond_6
    invoke-virtual {p0, v2}, Lcar;->d(I)V

    invoke-virtual {p0}, Lcar;->b()I

    move-result v1

    const/4 v3, 0x2

    if-ge v1, v3, :cond_7

    invoke-virtual {p0, v0}, Lcar;->c(I)V

    return-void

    :cond_7
    invoke-virtual {p0}, Lcar;->i()I

    move-result v1

    invoke-virtual {p0}, Lcar;->c()I

    move-result v3

    invoke-virtual {p0}, Lcar;->d()I

    move-result v4

    add-int/2addr v4, v1

    invoke-static {v3, v4}, Ljava/lang/Math;->min(II)I

    move-result v1

    invoke-virtual {p0, v1}, Lcar;->c(I)V

    goto :goto_0

    :cond_8
    :goto_1
    return-void
.end method


# virtual methods
.method public a(Lbmp;JLbmi$c;)Lbmi$f;
    .locals 6

    invoke-interface {p1}, Lbmp;->c()J

    move-result-wide v4

    invoke-interface {p1}, Lbmp;->d()J

    move-result-wide v0

    sub-long/2addr v0, v4

    const-wide/16 v2, 0x4e20

    invoke-static {v2, v3, v0, v1}, Ljava/lang/Math;->min(JJ)J

    move-result-wide v0

    long-to-int p4, v0

    iget-object v0, p0, Lbpr$a;->b:Lcar;

    invoke-virtual {v0, p4}, Lcar;->a(I)V

    iget-object v0, p0, Lbpr$a;->b:Lcar;

    iget-object v0, v0, Lcar;->a:[B

    const/4 v1, 0x0

    invoke-interface {p1, v0, v1, p4}, Lbmp;->c([BII)V

    iget-object v1, p0, Lbpr$a;->b:Lcar;

    move-object v0, p0

    move-wide v2, p2

    invoke-direct/range {v0 .. v5}, Lbpr$a;->a(Lcar;JJ)Lbmi$f;

    move-result-object p1

    return-object p1
.end method

.method public a()V
    .locals 2

    iget-object v0, p0, Lbpr$a;->b:Lcar;

    sget-object v1, Lcbf;->f:[B

    invoke-virtual {v0, v1}, Lcar;->a([B)V

    return-void
.end method
